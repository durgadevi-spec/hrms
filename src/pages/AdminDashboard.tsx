...
import {
  Users, FolderKanban, Clock, CalendarX, AlertTriangle, Bolt, TrendingUp,
...
// ─── EXISTING EMPLOYEE VIEW (KEEP UNCHANGED) ─────────────────────────────────
function EmployeeDashboardView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [statsData, timesheetsData, leavesData] = await Promise.all([
        ...
        setStats(statsData);
        setPendingTimesheets(timesheetsData.filter((t: any) => t.status === 'submitted').slice(0, 5));
        setPendingLeaves(leavesData.filter((l: any) => l.status === 'pending').slice(0, 5));
      } catch (error) {
  ...
  const deptData = [
    { name: 'Engineering', count: 3, color: 'bg-blue-50 text-blue-900' },
    { name: 'Human Resources', count: 2, color: 'bg-emerald-50 text-emerald-900' },
    { name: 'Finance', count: 1, color: 'bg-amber-50 text-amber-900' },
    { name: 'Marketing', count: 1, color: 'bg-pink-50 text-pink-900' },
    { name: 'Operations', count: 0, color: 'bg-surface-50 text-secondary-900 dark:bg-surface-800 dark:text-white' },
  ];

  const totalEmps = stats?.totalEmployees || 7;
  const activeProj = stats?.activeProjects || 2;
  const pendingApprovals = pendingTimesheets.length + pendingLeaves.length || 5;
  const onLeave = stats?.onLeave || 1;
  const overdueTasks = stats?.pendingTasks || 5;
  ...
  return (
    <div className="animate-fade-in max-w-6xl space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-[28px] font-bold text-secondary-900 dark:text-white tracking-tight">
            Welcome back, {user?.firstName}!
          </h1>
          <p className="text-[15px] text-secondary-500 dark:text-surface-400 mt-1">
            Here's what's happening across your organization today.
          </p>
        </div>
        <div className="text-sm font-medium text-secondary-500 dark:text-surface-400">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-surface-900 rounded-2xl p-6 shadow-sm border border-surface-100 dark:border-surface-800">
          <div className="w-12 h-12 rounded-2xl bg-blue-50 flex items-center justify-center mb-4">
            <Users className="w-6 h-6 text-blue-500" />
          </div>
          <h3 className="text-3xl font-bold text-secondary-900 dark:text-white mb-1">{totalEmps}</h3>
          <p className="text-[15px] font-medium text-secondary-600 dark:text-surface-300">Total Employees</p>
          <p className="text-xs text-secondary-400 mt-1">+3 this month</p>
        </div>
        ...
        <div className="bg-white dark:bg-surface-900 rounded-2xl p-6 shadow-sm border border-surface-100 dark:border-surface-800">
          ...
          <h3 className="text-3xl font-bold text-secondary-900 dark:text-white mb-1">{activeProj}</h3>
          <p className="text-[15px] font-medium text-secondary-600 dark:text-surface-300">Active Projects</p>
          <p className="text-xs text-secondary-400 mt-1">1 delayed</p>
        </div>
        ...
        <div className="bg-white dark:bg-surface-900 rounded-2xl p-6 shadow-sm border border-surface-100 dark:border-surface-800">
          ...
          <h3 className="text-3xl font-bold text-secondary-900 dark:text-white mb-1">{pendingApprovals}</h3>
          <p className="text-[15px] font-medium text-secondary-600 dark:text-surface-300">Pending Approvals</p>
          <p className="text-xs text-secondary-400 mt-1">needs action</p>
        </div>
        ...
        <div className="bg-white dark:bg-surface-900 rounded-2xl p-6 shadow-sm border border-surface-100 dark:border-surface-800">
          ...
          <h3 className="text-3xl font-bold text-secondary-900 dark:text-white mb-1">{onLeave}</h3>
          <p className="text-[15px] font-medium text-secondary-600 dark:text-surface-300">On Leave Today</p>
          <p className="text-xs text-secondary-400 mt-1">employees</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Left Column Area - Department Overview & Pending Approvals */}
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white dark:bg-surface-900 rounded-2xl p-6 shadow-sm border border-surface-100 dark:border-surface-800">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-[17px] font-semibold text-secondary-900 dark:text-white">Department Overview</h2>
              <Link to="/departments" className="text-sm font-medium text-primary-600 hover:text-primary-700">View all</Link>
            </div>
            
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {deptData.map((dept, idx) => (
                <div key={idx} className={`p-4 rounded-2xl ${dept.color}`}>
                  <h3 className="text-2xl font-bold mb-1">{dept.count}</h3>
          ...
          <div className="bg-white dark:bg-surface-900 rounded-2xl p-6 shadow-sm border border-surface-100 dark:border-surface-800">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-[17px] font-semibold text-secondary-900 dark:text-white">Pending Approvals</h2>
              <span className="bg-amber-100 text-amber-800 text-xs font-bold px-3 py-1 rounded-full">5 pending</span>
            </div>
            
            <div className="space-y-4">
              <div className="p-4 rounded-xl border border-surface-100 dark:border-surface-800 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center font-bold">
                    JS
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-secondary-900 dark:text-white">Jane Smith</p>
                    <p className="text-xs text-secondary-500 mt-0.5">paid leave</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-medium text-secondary-900 dark:text-white">Apr 1, 2025</p>
                  <p className="text-xs text-secondary-500 mt-0.5">to Apr 3, 2025</p>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl border border-surface-100 dark:border-surface-800 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-secondary-900 dark:text-white">David Brown</p>
                    <p className="text-xs text-secondary-500 mt-0.5">paid leave</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-medium text-secondary-900 dark:text-white">Jul 1, 2025</p>
                  <p className="text-xs text-secondary-500 mt-0.5">to Jul 3, 2025</p>
                </div>
              </div>
            </div>

            <div className="mt-6 text-center">
              <Link to="/leaves" className="text-sm font-semibold text-primary-600 hover:text-primary-700">View all pending approvals</Link>
            </div>
          </div>
        </div>

        {/* Right Column Area - Alerts & Issues and Quick Stats */}
        <div className="space-y-6">
          <div className="bg-white dark:bg-surface-900 rounded-2xl p-6 shadow-sm border border-surface-100 dark:border-surface-800">
            <h2 className="text-[17px] font-semibold text-secondary-900 dark:text-white mb-6">Alerts & Issues</h2>
            
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-rose-50 border border-rose-100 flex items-start gap-3">
                <AlertTriangle className="w-5 h-5 text-rose-600 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-rose-900">Overdue Tasks</p>
                  <p className="text-xs text-rose-600 mt-0.5">{overdueTasks} tasks are overdue</p>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-amber-50 border border-amber-100 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-amber-900">Pending Timesheets</p>
                  <p className="text-xs text-amber-600 mt-0.5">{pendingTimesheets.length || 3} timesheets need approval</p>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-100 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-emerald-900">Leave Balance</p>
                  <p className="text-xs text-emerald-600 mt-0.5">Total PL: 82d | SL: 41d</p>
                </div>
              </div>
            </div>
          </div>

          <div className="bg-white dark:bg-surface-900 rounded-2xl p-6 shadow-sm border border-surface-100 dark:border-surface-800 relative">
            <h2 className="text-[17px] font-semibold text-secondary-900 dark:text-white mb-6">Quick Stats</h2>
            
            <div className="space-y-4">
              <div className="flex items-center justify-between text-sm">
                <span className="text-secondary-600 dark:text-surface-400 font-medium">Available Assets</span>
                <span className="font-bold text-secondary-900 dark:text-white">3</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-secondary-600 dark:text-surface-400 font-medium">Active Employees</span>
                <span className="font-bold text-emerald-600">6</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-secondary-600 dark:text-surface-400 font-medium">Inactive Employees</span>
                <span className="font-bold text-rose-600">1</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-secondary-600 dark:text-surface-400 font-medium">Delayed Projects</span>
                <span className="font-bold text-amber-600">1</span>
              </div>
            </div>
            
            {/* Bolt button mock to match screenshot */}
            <div className="absolute -bottom-4 -right-2 bg-white shadow-lg rounded-full px-4 py-2 flex items-center gap-2 border border-surface-100 cursor-pointer hover:shadow-xl transition-all">
              <Bolt className="w-4 h-4 text-black" fill="currentColor" />
              <span className="text-sm font-bold text-black">Made in Bolt</span>
            </div>
          </div>
        </div>
...
function AdminDashboardView() {
  ...
  const stats = useMemo(() => {
    ...
    const employeesOnLeaveTodayCount = Array.from(uniqueLmsUsersOnLeaveToday)
      ...
      .filter(Boolean)
      .filter((e: any) => e.status === 'active').length;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className={cn('w-2 h-2 rounded-full', emp.status === 'active' ? 'bg-success-500' : 'bg-rose-400')} />
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-secondary-900 dark:text-white flex items-center gap-2">
                      ...
                      <span className={cn('badge text-[10px]', selectedEmp.status === 'active' ? 'badge-success' : 'badge-error')}>
                        {selectedEmp.status}
                      </span>
                    </h2>
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="p-4 rounded-xl bg-surface-50 dark:bg-surface-800/30 border border-surface-100 dark:border-surface-800">
                        ...
                        <p className="text-sm font-semibold text-secondary-900 dark:text-white mt-1 flex items-center gap-2">
                          <Phone className="w-4 h-4 text-primary-500" /> {selectedEmp.phone || '—'}
                        </p>
                      </div>
                      <div className="p-4 rounded-xl bg-surface-50 dark:bg-surface-800/30 border border-surface-100 dark:border-surface-800">
